...
打开讲义点击「段落批注」，选择段落即可添加个人笔记；左侧「我的批注」支持查看、编辑、删除，以及跳转并高亮原讲义段落。管理员也不能通过批注接口访问他人的笔记。绑定规则、权限、备份限制和隔离预览见 [个人批注](docs/个人批注.md)。

个人批注不包含在管理员下载的资料备份中；存在批注时应用内整库恢复会被阻止，需要服务器维护者通过数据库备份处理。该功能尚未部署生产或重新打包。

## 第二版：经文检索与关联讲义

登录后点击左侧「经文检索」，输入关键词并按回车或点击「检索」。搜索覆盖资料库已录入的全部译本，结果高亮关键词，并显示完整卷名、章、节和译本名称。每次显示 100 条，通过「加载更多」可查看全部匹配结果。

点击结果后，右侧抽屉列出引用该经节的全部可见讲义，包含目录及每篇讲义的完整正文；点击目录可跳转到对应正文。连续经节范围和半节标记均按包含该节处理，同一讲义重复引用只列出一次。关闭按钮或抽屉内按 Escape 可关闭抽屉。

关联依据为讲义当前保存正文的引用索引，保存和历史恢复后随之更新，不搜索已被修改掉的历史引用。管理员可查看未删除的已发布及未发布讲义，阅读用户只能查看已发布且未删除的讲义。经文和讲义均来自当前资料库，未录入的内容不会自动补齐。

接口：`GET /api/verses/search?q=创造&offset=0&limit=100` 返回结果及总数；`GET /api/verses/lectures?book=Gen&chapter=1&verse=1` 返回关联讲义全文。两者均需应用密钥及登录认证。
