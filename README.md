...
没有默认正式管理员或内置正式经文。首次管理员由使用者自行设置。初始密码至少 10 位；添加用户后首次登录必须改密码。测试账户只存在于 `/private/tmp` 的隔离预览数据库中。
...
经文管理页面支持两约、书卷、章节浏览和逐节维护。字段解释、关联约束及采集程序写入示例见 [圣经数据结构与爬虫接口](docs/圣经数据结构与爬虫接口.md)。本次新增迁移为 `0002`，未实现网站采集；管理页不提供 JSON 导入入口，既有后端导入接口保留兼容。
