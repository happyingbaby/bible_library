...
export default function ScriptureManager({translations,onRefresh,onDirty,onBusy}:Props){
  ...
  const [edit,setEdit]=useState<{verse:number;text:string;original:string;revision:number|null}|null>(null);
  ...
  const openVerse=(number?:number)=>navigate(()=>{
    ...
    setEdit({verse,text:row?.text||'',original:row?.text||'',revision:content?.revision??null});
  });
  ...
  const save=()=>run(async()=>{
    ...
    if(!edit.original&&content.verses.some(v=>v.verse===edit.verse))throw Error('所选译本的该节已有正文，请取消后使用该节的编辑按钮核对修改');
  ...
  return <div className="bible-manager">
    ...
    <section className="bible-content"><h3>{current?.name||book} <span className="muted">/ {current?.chapters||'—'} 章</span></h3>
      <nav className="chapter-navigation" aria-label="章节导航"><button disabled={busy||loading||chapter<=1} onClick={()=>navigate(()=>setChapter(chapter-1))}>上一章</button><strong>第 {chapter} 章</strong><button disabled={busy||loading||chapter>=(current?.chapters||1)} onClick={()=>navigate(()=>setChapter(chapter+1))}>下一章</button></nav>
      <div className="bible-section-heading"><h3>选择经节</h3><button disabled={busy||loading} onClick={()=>navigate(()=>{setRange(null);setAnchor(null);})}>显示全章</button></div>
      ...
      {edit&&<form ref={editRef} className="inset bible-edit" onSubmit={e=>{e.preventDefault();save();}}><h3>{edit.original?'编辑经文':'录入经文'} · {current?.name} {chapter}:{edit.verse}</h3><label className="field"><span>所属译本</span><Choice label="经文所属译本" disabled={busy||!!edit.original} value={translation} onChange={value=>{setContent(null);setLoading(true);setEdit({...edit,revision:null});setTranslation(value);}} options={[{value:'',label:'请选择所属译本'},...translations.map(t=>({value:String(t.id),label:`${t.name} · ${t.language}`}))]}/></label>{!translation&&<p className="callout neutral">{translations.length?'可先输入正文，再选择译本保存。':'当前尚无译本。可以先填写正文，再点击上方「新建译本」；正文草稿会保留。'}</p>}{!edit.original&&content?.verses.some(v=>v.verse===edit.verse)&&<p className="callout">该译本的此节已有正文，请通过该节的「编辑」按钮核对修改。</p>}<label className="field"><span>节号</span><input type="number" required min={1} max={176} value={edit.verse} disabled={busy||!!edit.original} onChange={e=>{const verse=Number(e.target.value);if(content?.verses.some(v=>v.verse===verse)){setError('该节已有正文，请在下方选择编辑');return;}setEdit({...edit,verse});}}/></label><label className="field"><span>正文</span><textarea autoFocus required maxLength={10000} rows={5} value={edit.text} disabled={busy} onChange={e=>setEdit({...edit,text:e.target.value})}/></label><div className="bible-edit-actions"><button className="primary" disabled={busy||loading||!translation||edit.revision===null||!edit.text.trim()||(!edit.original&&!!content?.verses.some(v=>v.verse===edit.verse))}>保存经文</button><button type="button" disabled={busy} onClick={()=>navigate(()=>setEdit(null))}>取消</button><button type="button" disabled={busy} onClick={()=>setRefresh(n=>n+1)}>刷新章节</button></div><p className="small muted">若发生版本冲突，刷新后取消编辑，再重新打开该节核对修改。</p></form>}
      {!loading&&<div className="bible-verses">{visibleNumbers.map(n=>{const row=content?.verses.find(v=>v.verse===n);return <VerseRow key={n}><span className="verse-number">{n}</span><p className={row?'':'muted'}>{row?.text||(translation?'尚未录入':'尚未选择译本，可先录入此节正文')}</p><div><button disabled={busy} onClick={()=>openVerse(n)}>{row?'编辑':'录入'}</button>{row&&content&&<button className="danger" disabled={busy} onClick={()=>{if(!guard()||!window.confirm(`确定删除${current?.name} ${chapter}:${n} 的正文吗？`))return;run(async()=>{await api(`/bible/translations/${translation}/${book}/${chapter}/${n}?revision=${content.revision}`,'DELETE');setEdit(null);setRefresh(v=>v+1);setNotice('经文已删除');await onRefresh();});}}>删除</button>}</div></VerseRow>;})}{!numbers.length&&<p className="callout neutral">本章尚无经文。可新增经节，后续也可由采集程序写入。</p>}</div>}
    </section></div>
  </div>;
}
